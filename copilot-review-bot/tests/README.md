...
stub server, and it skips itself, loudly, without it.

The `test` job in [`copilot-review-bot.yml`](../../.github/workflows/copilot-review-bot.yml) runs
all five on every push and pull request that touches `copilot-review-bot/`. That job also asserts
that `python3` is present, so the skip above cannot pass silently in CI.
...
|----------------------------------|----------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `copilot-review-bot-test.sh`     | The decision rules                                 | Merge against non-merge commits, unresolved threads, all six `basis` values, pending requests, review thread ownership, and every mention case. It includes the restack regression, with the real author and committer dates from `XRPLF/rippled#7941`.                     |
| `copilot-review-bot-e2e-test.sh` | The whole bot                                      | What the bot actually did: which mutations it sent, what it logged, and what it exited with. This is where the per-run cap, the transient and permanent split, the reactions, option validation, the read-failure breaker, the run deadline, and the log shape are covered. |
| `copilot-review-bot-gcs-test.sh` | The `gs://` state backend                          | The lock, which is the only thing between two overlapping executions and a lost write. It covers create, conflict, stale break, both lost races, release, the state round trip, a missing bucket permission, and the metadata server path Cloud Run uses.                   |
...
| `deploy-job-test.sh`             | `deploy/deploy-job.sh` and `deploy/rate-budget.sh` | The `gcloud` commands each would run. These two are the only files here that change production, so the suite covers the create and update paths, the replaced environment set, the `jobs.json` schema, the quota arithmetic, and the timeout ordering.                      |
...
Four rules are shared by every suite, and they are what make the results repeatable:

1. **`PATH` is rebuilt from scratch.** `build_tool_path` in `test-lib.sh` locates each tool it needs
   and puts the stub directory first. A stub cannot be bypassed, and a macOS machine gets its
...
5. **Retry backoff is 0.** Both `GH_RETRY_BASE_SECONDS` and `GCS_RETRY_BASE_SECONDS` are set to 0 in
   the suite that exercises them. Every assertion is on the retry having happened, and none is on
   how long it waited, so the production defaults would only add sleep. Set them in the suite, never
   in the workflow: `env -i` means a value from outside never arrives.
...
appends one line per call to `calls.log`. The suite then asserts on the mutations that were sent
rather than on the ones the bot said it sent. The stub Cloud Storage server implements only what the
bot uses, but it implements `ifGenerationMatch` on create and on delete faithfully, because that
...
* A change to what counts as a mention belongs there too. Each of the four exclusions has a case: a
  block quote, a fenced block, an indented block and an inline code span. A fifth kind of text that
  must not count needs a fifth case, because the filter is one jq expression and nothing else covers
  it.
...
* A change to `jobs.json`, its schema or the quota arithmetic belongs in `deploy-job-test.sh`. That
  suite already asserts that the checked-in `jobs.json` passes both scripts, so a new field needs a
  case there and in the schema.
...
changes nothing, and it needs no token, so it is the tool for a post-mortem on a PR that behaved
unexpectedly.
...
mode, and they are what you need when the question is about a mention rather than a commit:
...
there, because a local filesystem persists:
...
macOS works for testing, with two caveats. Put a Homebrew bash first on `PATH`, because `/bin/bash`
is still 3.2. Run `brew install flock`, because a local state root locks with `flock`. BSD `date`
needs nothing: the flavor is probed at startup.

### Never point a live local run at the production state root

A real run takes the lock before it evaluates any PR. A local run against the production prefix
therefore takes the lock the scheduled job needs, and deletes it on release. Every tick in the
meantime logs `run.skipped`.

`--dry-run` is exempt, because it takes no lock, local or `gs://`. It is safe to point at the
production prefix by hand.

It is not free, though. A dry run reads every open PR, so against `XRPLF/rippled` it spends roughly
538 GraphQL points, near 11% of the hourly quota the scheduled fleet is already drawing on. Prefer
`--pr N` when one PR is the question, and see [A dry run measures the
...
The prefix is what keeps you clear of production. It is optional, and the deployed jobs leave it
out, so `gs://<bucket>` **is** the production state root. Adding a segment of your own puts your
lock and markers somewhere no scheduled job looks.

## What no test here can prove

Every suite stubs `gh pr edit`, so no suite reaches the one mutation that matters. See [Asking
Copilot, by name](../README.md#asking-copilot-by-name) for what that leaves unverified. The only way
to check it is to watch a real request land.
